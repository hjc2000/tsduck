...
namespace ts
{
	...
	public:
		...
		//!
		ReportFile(UString const &file_name, bool append = false, int max_severity = Severity::Info);
	...
	protected:
		virtual void writeLog(int severity, UString const &message) override;
...
ts::ReportFile<MUTEX>::ReportFile(UString const &file_name, bool append, int max_severity)
	: Report(max_severity),
...
template <class MUTEX>
void ts::ReportFile<MUTEX>::writeLog(int severity, UString const &message)
{
	std::lock_guard<MUTEX> lock(_mutex);
