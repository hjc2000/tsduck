...
public:                                           \
	...
	classname &operator=(const classname &) = default /** @endcond */
...
public:                                                \
	...
	virtual ~classname() dtor_attributes /** @endcond */
...
#define TS_STATIC_REFERENCE(suffix, addr)                                            \
	static volatile const void *volatile const TS_UNIQUE_NAME(ref##suffix) = (addr); \
	static volatile const void *volatile const TS_UNIQUE_NAME(ref2##suffix) = &TS_UNIQUE_NAME(ref##suffix)
#endif
...
namespace ts
{
	...
	typedef char const *const_char_ptr;       //!< Pointer to @c const char
	typedef int8_t const *const_int8_ptr;     //!< Pointer to @c const int8_t
	typedef int16_t const *const_int16_ptr;   //!< Pointer to @c const int16_t
	typedef int32_t const *const_int32_ptr;   //!< Pointer to @c const int32_t
	typedef int64_t const *const_int64_ptr;   //!< Pointer to @c const int64_t
	typedef uint8_t const *const_uint8_ptr;   //!< Pointer to @c const uint8_t
	typedef uint16_t const *const_uint16_ptr; //!< Pointer to @c const uint16_t
	typedef uint32_t const *const_uint32_ptr; //!< Pointer to @c const uint32_t
	typedef uint64_t const *const_uint64_ptr; //!< Pointer to @c const uint64_t
	...
	{
		return Tristate(std::max<INT>(-1, std::min<INT>(1, i)));
	}
	...
	inline void set_default(std::optional<T> &opt, U const &value)
	{
		if (!opt)
		{
			opt = value;
...
{
	template <>
	class lock_guard<ts::null_mutex>
	{
	public:
