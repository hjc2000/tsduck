...
namespace ts
{
	...
	protected:
		...
		//!
		AbstractLongTable(TID tid, UChar const *xml_name, Standards standards, uint8_t version_, bool is_current_);

		// Inherited methods.
		virtual size_t maxPayloadSize() const override;
		virtual bool useTrailingCRC32() const override;
		virtual void deserializePayloadWrapper(PSIBuffer &, Section const &) override;
