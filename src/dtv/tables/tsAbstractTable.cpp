...
ts::AbstractTable::AbstractTable(TID tid, UChar const *xml_name, Standards standards, UChar const *xml_legacy_name)
...
ts::AbstractTable::EntryWithDescriptors::EntryWithDescriptors(AbstractTable const *table)
	: EntryBase(NPOS),
	  descs(table)
{
}

ts::AbstractTable::EntryWithDescriptors::EntryWithDescriptors(AbstractTable const *table, EntryWithDescriptors const &other)
	: EntryBase(NPOS),
	  descs(table, other.descs)
{
}

ts::AbstractTable::EntryWithDescriptors::EntryWithDescriptors(AbstractTable const *table, EntryWithDescriptors &&other)
	: EntryBase(NPOS),
	  descs(table, other.descs)
{
}

ts::AbstractTable::EntryWithDescriptors &ts::AbstractTable::EntryWithDescriptors::operator=(EntryWithDescriptors const &other)
{
	if (&other != this)
	{
...
bool ts::AbstractTable::serialize(DuckContext &duck, BinaryTable &table) const
{
	...
	else
	{
		...
		{
			size_t const current_write = payload.currentWriteByteOffset();
			payload.swapState();
...
void ts::AbstractTable::addOneSectionImpl(BinaryTable &table, PSIBuffer &payload) const
{
	...
	{
		SectionPtr const section(new Section(tableId(), isPrivate(), payload.currentReadAddress(), payload.remainingReadBytes()));
...
bool ts::AbstractTable::deserialize(DuckContext &duck, BinaryTable const &table)
{
	...
	for (size_t si = 0; si < table.sectionCount(); ++si)
	{
		...
		Section const &section(*table.sectionAt(si));
		assert(section.isValid());

		// Check if we shall manually check the value of a CRC32 in a short section.
		bool const short_crc = section.isShortSection() && useTrailingCRC32();
		if (short_crc)
		{
...
void ts::AbstractTable::deserializePayloadWrapper(PSIBuffer &buf, Section const &section)
{
