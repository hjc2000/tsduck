...
ts::AbstractLongTable::AbstractLongTable(TID tid, UChar const *xml_name, Standards standards, uint8_t version_, bool is_current_)
...
void ts::AbstractLongTable::deserializePayloadWrapper(PSIBuffer &buf, Section const &section)
{
...
void ts::AbstractLongTable::addOneSectionImpl(BinaryTable &table, PSIBuffer &payload) const
{
	...
	{
		// Add one section.
		uint8_t const section_number = uint8_t(table.sectionCount());
		SectionPtr const section(new Section(tableId(),
											 isPrivate(),
											 tableIdExtension(),
											 version,
											 is_current,
