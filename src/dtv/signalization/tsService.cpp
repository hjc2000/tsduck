...
void ts::Service::set(UString const &desc)
{
	clear();
...
bool ts::Service::match(UString const &ident, bool exact_match) const
{
	uint16_t id = 0;
	uint16_t minor = 0;
...
bool ts::Service::Sort1(Service const &s1, Service const &s2)
{
	_SORT_(_lcn)
	_SORT_(_onid)
	_SORT_(_tsid)
...
bool ts::Service::Sort2(Service const &s1, Service const &s2)
{
	_SORT_(_name)
	_SORT_(_provider)
	_SORT_(_lcn)
...
bool ts::Service::Sort3(Service const &s1, Service const &s2)
{
	_SORT_(_onid)
	_SORT_(_tsid)
	_SORT_(_id)
