...
namespace ts
{
	...
	public:
		...
		//!
		Section(Section const &other, ShareMode mode);
		...
		//! @param [in] crc_op How to process the CRC32.
		//!
		Section(ByteBlock const &content,
				PID source_pid = PID_NULL,
				CRC32::Validation crc_op = CRC32::Validation::IGNORE);
		...
		//! @param [in] crc_op How to process the CRC32.
		//!
		Section(ByteBlockPtr const &content_ptr,
				PID source_pid = PID_NULL,
				CRC32::Validation crc_op = CRC32::Validation::IGNORE);
		...
		virtual void reload(void const *content, size_t content_size, PID source_pid = PID_NULL) override;
		virtual void reload(ByteBlock const &content, PID source_pid = PID_NULL) override;
		virtual void reload(ByteBlockPtr const &content_ptr, PID source_pid = PID_NULL) override;
		...
		//! @param [in] crc_op How to process the CRC32.
		//!
		void reload(void const *content, size_t content_size, PID source_pid, CRC32::Validation crc_op);
		...
		//! @param [in] crc_op How to process the CRC32.
		//!
		void reload(ByteBlock const &content, PID source_pid, CRC32::Validation crc_op);
		...
		//! @param [in] crc_op How to process the CRC32.
		//!
		void reload(ByteBlockPtr const &content_ptr, PID source_pid, CRC32::Validation crc_op);
		...
		//! @param [in] source_pid PID from which the section was read.
		//!
		void reload(TID tid, bool is_private_section, void const *payload, size_t payload_size, PID source_pid = PID_NULL);
		...
		//! @return A reference to this object.
		//!
		Section &operator=(Section const &&other) noexcept;
		...
		//! @return A reference to this object.
		//!
		Section &copy(Section const &other);
		...
		TID tableId() const
		{
			return _is_valid ? content()[0] : uint8_t(TID_NULL);
		}
		...
		//!
		static bool StartLongSection(uint8_t const *data, size_t size);
		...
		{
			return _is_valid && StartLongSection(content(), size());
		}
		...
		{
			return _is_valid && (content()[1] & 0x40) != 0;
		}
		...
		{
			return isLongSection() ? GetUInt16(content() + 3) : 0;
		}
		...
		{
			return isLongSection() ? ((content()[5] >> 1) & 0x1F) : 0;
		}
		...
		{
			return isLongSection() && (content()[5] & 0x01) != 0;
		}
		...
		{
			return isLongSection() ? content()[6] : 0;
		}
		...
		{
			return isLongSection() ? content()[7] : 0;
		}
		...
		{
			return SectionPacketCount(size());
		}
		...
		//! @param [in] recompute_crc If true, immediately recompute the CRC32 of the section.
		//!
		void appendPayload(void const *data, size_t size, bool recompute_crc = true);
		...
		//! @param [in] recompute_crc If true, immediately recompute the CRC32 of the section.
		//!
		void appendPayload(ByteBlock const &data, bool recompute_crc = true)
		{
		...
		//!
		static size_t SectionSize(void const *content, size_t content_size);
		...
		//!
		static size_t SectionSize(ByteBlock const &content)
		{
			return SectionSize(content.data(), content.size());
		}
