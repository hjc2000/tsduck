...
namespace ts
{
	...
	public:
		...
		//!
		explicit ETID(TID tid = 0xFF)
			: _etid((uint32_t(tid) & 0xFF) << 16)
		{
		}
		...
		ETID(TID tid, uint16_t tid_ext)
			: _etid(0x01000000 | ((uint32_t(tid) & 0xFF) << 16) | (uint32_t(tid_ext) & 0xFFFF))
		{
		}
