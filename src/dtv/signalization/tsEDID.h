...
namespace ts
{
	...
	public:
		...
		{
			return EDID(0xFFFFFFFFFFFFFF00 | (did & 0xFF));
		}
		...
		{
			return EDID((uint64_t(pds) << 32) | 0x00000000FFFFFF00 | (did & 0xFF));
		}
		...
		{
			return EDID(0xFFFFFFFFFFFF0000 | (uint64_t(ext & 0xFF) << 8) | uint64_t(DID_DVB_EXTENSION));
		}
		...
		{
			return EDID(0xFFFFFFFFFFFF0000 | (uint64_t(ext & 0xFF) << 8) | uint64_t(DID_MPEG_EXTENSION));
		}
		...
		{
			return EDID(0xFFFFFFFFFF00FF00 | (uint64_t(tid & 0xFF) << 16) | (did & 0xFF));
		}
		...
		{
			return (_edid & 0xFFFFFFFFFFFFFF00) == 0xFFFFFFFFFFFFFF00;
		}
		...
		PDS pds() const
		{
			return did() >= 0x80 ? PDS((_edid >> 32) & 0xFFFFFFFF) : PDS(PDS_NULL);
		}
		...
		{
			return didExtMPEG() != MPEG_EDID_NULL;
		}
		...
		DID didExtDVB() const
		{
			return did() == DID_DVB_EXTENSION ? DID((_edid >> 8) & 0xFF) : DID(EDID_NULL);
		}
		...
		{
			return did() == DID_MPEG_EXTENSION ? DID((_edid >> 8) & 0xFF) : DID(MPEG_EDID_NULL);
		}
