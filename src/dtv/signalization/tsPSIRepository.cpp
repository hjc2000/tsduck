...
FUNCTION ts::PSIRepository::getTableFunction(TID tid, Standards standards, PID pid, uint16_t cas, FUNCTION TableDescription::*member) const
{
	...
	for (auto it = _tables.lower_bound(tid); it != _tables.end() && it->first == tid; ++it)
	{
		...
		if (it->second.*member != nullptr)
		{
			...
			bool const casMatch = cas >= it->second.minCAS && cas <= it->second.maxCAS;

			// Standard match: at least one standard of the table is current, or standard-agnostic table (Standards::NONE).
			bool const stdMatch = bool(standards & it->second.standards) || it->second.standards == Standards::NONE;
...
ts::PSIRepository::RegisterXML::RegisterXML(UString const &filename)
{
...
ts::PSIRepository::RegisterTable::RegisterTable(TableFactory factory,
												std::vector<TID> const &tids,
												Standards standards,
												UString const &xmlName,
...
ts::PSIRepository::RegisterTable::RegisterTable(std::vector<TID> const &tids,
												Standards standards,
												DisplaySectionFunction displayFunction,
												LogSectionFunction logFunction,
												std::initializer_list<PID> pids,
...
void ts::PSIRepository::RegisterDescriptor::registerXML(DescriptorFactory factory, const EDID &edid, UString const &xmlName, UString const &xmlNameLegacy)
{
	PSIRepository &repo(PSIRepository::Instance());
...
ts::PSIRepository::TableFactory ts::PSIRepository::getTableFactory(UString const &node_name) const
{
	auto const it = node_name.findSimilar(_tableNames);
	return it != _tableNames.end() ? it->second : nullptr;
}

ts::PSIRepository::DescriptorFactory ts::PSIRepository::getDescriptorFactory(UString const &node_name) const
{
	auto const it = node_name.findSimilar(_descriptorNames);
...
{
	auto const it = _casIdDescriptorDisplays.find(cas_id);
...
bool ts::PSIRepository::isDescriptorAllowed(UString const &desc_node_name, TID table_id) const
{
...
ts::UString ts::PSIRepository::descriptorTables(DuckContext const &duck, UString const &desc_node_name) const
{
	auto it = desc_node_name.findSimilar(_descriptorTablesIds);
	UString result;
...
{
	ids.clear();
	for (auto const &it : _descriptors)
	{
		ids.push_back(it.first);
	}
}
