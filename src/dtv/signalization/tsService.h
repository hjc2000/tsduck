...
namespace ts
{
	...
	public:
		...
		//! this is a service id, otherwise this is a service name.
		//!
		virtual void set(UString const &desc);
...
public:                                                           \
	...
	void clear##suffix()                                          \
	{                                                             \
		_modified = _modified || bool(field);                     \
		field.reset();                                            \
	}                                                             \
	/** Get the fullname.                                   */    \
	...
	{                                                             \
		return field.value_or(defvalue);                          \
	}
...
#define SERVICE_PROPERTY_INT(type, suffix, field, defvalue, fullname) \
	...
	{                                                                 \
		_modified = _modified || field != value;                      \
		field = value;                                                \
	}                                                                 \
...
#define SERVICE_PROPERTY_STRING(suffix, field, fullname)          \
	...
	{                                                             \
		return value.similar(field.value_or(UString()));          \
	}
...
#undef SERVICE_PROPERTY_STRING
		...
		//!
		bool match(UString const &ident, bool exact_match = false) const;
		...
		//!
		static bool Sort1(Service const &s1, Service const &s2);
		...
		//! @return True is @a s1 is logically less than @a s2, false otherwise.
		//!
		static bool Sort2(Service const &s1, Service const &s2);
		...
		//! @return True is @a s1 is logically less than @a s2, false otherwise.
		//!
		static bool Sort3(Service const &s1, Service const &s2);
		...
		static std::ostream &Display(std::ostream &strm, UString const &margin, const CONTAINER &container, bool header = true)
		{
...
{
	// Some header
	UString const h_name(u"Name");
	UString const h_provider(u"Provider");
