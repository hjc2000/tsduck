...
ts::Section::Section(Section const &sect, ShareMode mode)
	: SuperClass(sect, mode),
	  _is_valid(sect._is_valid)
{
}

ts::Section::Section(void const *content, size_t content_size, PID source_pid, CRC32::Validation crc_op)
...
ts::Section::Section(ByteBlock const &content, PID source_pid, CRC32::Validation crc_op)
...
ts::Section::Section(ByteBlockPtr const &content_ptr, PID source_pid, CRC32::Validation crc_op)
...
ts::Section::Section(TID tid, bool is_private_section, void const *payload, size_t payload_size, PID source_pid)
	: Section()
{
...
void ts::Section::reload(void const *content, size_t content_size, PID source_pid)
{
	SuperClass::reload(content, content_size, source_pid);
	validate(CRC32::CHECK);
}

void ts::Section::reload(ByteBlock const &content, PID source_pid)
{
	SuperClass::reload(content, source_pid);
	validate(CRC32::CHECK);
}

void ts::Section::reload(ByteBlockPtr const &content_ptr, PID source_pid)
{
	SuperClass::reload(content_ptr, source_pid);
	validate(CRC32::CHECK);
}

void ts::Section::reload(void const *content, size_t content_size, PID source_pid, CRC32::Validation crc_op)
{
	SuperClass::reload(content, content_size, source_pid);
	validate(crc_op);
}

void ts::Section::reload(ByteBlock const &content, PID source_pid, CRC32::Validation crc_op)
{
	SuperClass::reload(content, source_pid);
	validate(crc_op);
}

void ts::Section::reload(ByteBlockPtr const &content_ptr, PID source_pid, CRC32::Validation crc_op)
{
	SuperClass::reload(content_ptr, source_pid);
	validate(crc_op);
}
...
void ts::Section::reload(TID tid, bool is_private_section, void const *payload, size_t payload_size, PID source_pid)
{
	clear();
...
size_t ts::Section::SectionSize(void const *content, size_t content_size)
{
	...
	{
		uint8_t const *data = reinterpret_cast<uint8_t const *>(content);
...
void ts::Section::validate(CRC32::Validation crc_op)
{
	...
	uint8_t const *const daddr = content();
	size_t const dsize = size();
	size_t const total_size = SectionSize(daddr, dsize);
	bool const is_long = StartLongSection(daddr, dsize);
	...
	if (is_long)
	{
		...
		size_t const sec_size = dsize - SECTION_CRC32_SIZE;
		switch (crc_op)
		{
		case CRC32::CHECK:
...
ts::Section &ts::Section::operator=(Section const &sect)
{
	if (&sect != this)
	{
...
ts::Section &ts::Section::operator=(Section const &&sect) noexcept
{
	if (&sect != this)
	{
...
ts::Section &ts::Section::copy(Section const &sect)
{
	if (&sect != this)
	{
...
bool ts::Section::operator==(Section const &sect) const
{
...
void ts::Section::recomputeCRC()
{
	...
	{
		// Section size, without CRC32:
		size_t const sec_size = size() - SECTION_CRC32_SIZE;
...
bool ts::Section::StartLongSection(uint8_t const *data, size_t size)
{
...
void ts::Section::appendPayload(void const *data, size_t dsize, bool recompute_crc)
{
	if (_is_valid && data != nullptr && dsize != 0)
	{
		...
		bool const is_long = isLongSection() && size() >= LONG_SECTION_HEADER_SIZE + 4;
		if (is_long)
		{
			rwResize(size() - 4);
		}
		...
		if (is_long)
		{
			static uint8_t const byte4[4] = {0, 0, 0, 0};
...
{
	size_t const previous_size = payloadSize();
	...
	if (_is_valid && dsize < previous_size)
	{
		...
		size_t const remove = previous_size - dsize;
...
std::ostream &ts::Section::write(std::ostream &strm, Report &report) const
{
	if (_is_valid && strm)
	{
		strm.write(reinterpret_cast<char const *>(content()), std::streamsize(size()));
		if (!strm)
		{
